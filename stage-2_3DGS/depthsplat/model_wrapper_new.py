...
class ModelWrapper(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        pred_depths = None  # Will be set if encoder returns depths
        ...
        # Use target foreground mask to only compute loss on foreground pixels
        # This prevents the model from collapsing to black output (background is ~90% of pixels)
        valid_depth_mask = None
        if "mask" in batch["target"]:
            target_mask = batch["target"]["mask"]  # [B, V, H, W] foreground mask
            if target_mask.dim() == 5:
                target_mask = target_mask.squeeze(2)  # [B, V, 1, H, W] -> [B, V, H, W]
            # valid_depth_mask=True means "exclude this pixel" - so we exclude background (mask < 0.5)
            # Expand to match image shape [B, V, C, H, W]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if not self.train_cfg.forward_depth_only:
            ...
            depth_softmax = output_softmax.depth[0]  # [V, H, W]
        ...
        if not self.train_cfg.forward_depth_only:
            if not self.train_cfg.no_log_projections:
                ...
                right_panel = vcat(
                    add_label(add_border(projections), "Projections"),
                    add_label(add_border(cameras), "Cameras")
                )
            else:
                right_panel = None
            ...
            def process_depth(depth_tensor):
                # depth_tensor: [V, H, W] or [H, W] (if V=1 and squeezed, but here we expect [V, H, W])
                # returns list of [3, H, W] float tensors in [0,1]
                
                # If depth is inverse depth (disparity) vs metric depth depends on model. 
                # viz_depth_tensor expects inverse depth usually or just scalar.
                # It does normalization internally.
                
                # viz_depth_tensor returns [3, H, W] byte tensor.
                # We convert to float [0,1].
                
                viz_list = []
                for d in depth_tensor:
                    # d: [H, W]
                    # We compute inverse here if needed, but viz_depth_tensor handles rescaling
                    # Let's pass 1/d if it's metric depth, or d if disparity.
                    # Assuming metric depth comes out of model/dataset usually?
                    # logic in existing code (line 691) does `1.0 / pred_depths`.
                    # So let's try 1.0 / d safely.
                    
                    # Avoid div by zero
                    d_inv = 1.0 / (d + 1e-6)
                    
                    viz = viz_depth_tensor(d_inv.cpu().detach(), return_numpy=False) # [3, H, W] byte
                    viz = viz.float() / 255.0
                    viz_list.append(viz)
                return viz_list

            # 1. Context Views
            context_imgs = [x for x in batch["context"]["image"][0]] # List of [3, H, W]
            
            # Check for context depth in pred_depths (encoder output)
            if pred_depths is not None:
                # pred_depths from encoder is already [V, H, W] usually (from line 680 logic)
                # But notice line 680: pred_depths = pred_depths[0]
                # And line 651: pred_depths = gaussians_softmax["depths"]
                # In encoder output it might be inside dict. 
                # I need to follow logic.
                
                # Existing logic at 678: if pred_depths is not None...
                # It does interpolation if needed.
                # I should reuse `pred_depths` if it was processed.
                
                # To be safe, I will re-process or use what is available.
                # `pred_depths` variable in this scope comes from line 648/651.
                # It is [B, V, H, W] initially? 
                # line 651: pred_depths = gaussians_softmax["depths"]
                # line 680 (in existing code): pred_depths = pred_depths[0]
                # I CANNOT RELY on line 680 execution because I am possibly editing before/after or taking over.
                # Wait, I am editing the block at 718+.
                # Code at 678 runs before 718.
                # So `pred_depths` might have been modified at 680 if it existed.
                # CAUTION: If 678 runs, `pred_depths` is `pred_depths[0]`.
                # If 678 does NOT run (pred_depths is None), then it is None.
                
                # However, `viz depth` block (678-716) updates `pred_depths` in place?
                # Yes: `pred_depths = pred_depths[0]`
                pass
            
            # Let's handle Context Depth
            context_depth_viz = []
            if pred_depths is not None:
                # Reuse pred_depths. It should be [V, H, W] or [H, W] depending on earlier run.
                # If it was [1, V, H, W] originally, line 680 made it [V, H, W].
                # If earlier code ran, fine.
                # I'll convert it to list.
                
                # Wait, if `pred_depths` was modified in lines 678-716, I can use it.
                # BUT, lines 678-716 are BEFORE my edit chunk (718+).
                # So `pred_depths` is valid [V, H, W] tensor if it exists.
                
                context_depth_viz = process_depth(pred_depths)
            else:
                # If no predicted context depth, maybe GT?
                if "depth" in batch["context"]:
                    context_depth_viz = process_depth(batch["context"]["depth"][0])
                else:
                    # Fill with black or empty
                    context_depth_viz = [torch.zeros_like(img) for img in context_imgs]

            # Combine Context Image + Depth
            # If we have more depth viz than images or vice versa, clip?
            # They should match V.
            context_col = vcat(*[hcat(img, d) for img, d in zip(context_imgs, context_depth_viz)])
            context_col = add_label(context_col, "Context")

            # 2. Target GT Views
            target_gt_imgs = [x for x in rgb_gt] # rgb_gt is [V, 3, H, W]
            
            target_gt_depth_viz = []
            # Check for depth in target
            # batch["target"] might have "depth"?
            # BatchedViews has "depth" key in some datasets? 
            # `BatchedViews` TypedDict (checked earlier) shows: extrinsics, intrinsics, image, near, far, index. 
            # It DOES NOT show "depth" in `types.py` I viewed earlier (Step 33).
            # But line 653: `depth_gt = batch["context"]["depth"]` implies context has it.
            # I should verify if target has it. Code doesn't use it elsewhere?
            # If not in TypedDict, it might still be in dict at runtime if dataset provides it.
            # I will check `batch["target"].get("depth")`.
            
            val_target_depth = batch["target"].get("depth")
            if val_target_depth is not None:
                target_gt_depth_viz = process_depth(val_target_depth[0])
            else:
                 target_gt_depth_viz = [torch.zeros_like(img) for img in target_gt_imgs]
            
            target_gt_col = vcat(*[hcat(img, d) for img, d in zip(target_gt_imgs, target_gt_depth_viz)])
            target_gt_col = add_label(target_gt_col, "Target (GT)")


            # 3. Target Pred Views
            target_pred_imgs = [x for x in rgb_softmax]
            target_pred_depth_viz = process_depth(depth_softmax)
            
            target_pred_col = vcat(*[hcat(img, d) for img, d in zip(target_pred_imgs, target_pred_depth_viz)])
            target_pred_col = add_label(target_pred_col, "Target (Pred)")

            
            # Combine Comparison
            comparison_panel = hcat(context_col, target_gt_col, target_pred_col)
            comparison_panel = add_border(comparison_panel)

            # Final Combination
            if right_panel is not None:
                # Resize right panel to match comparison height? 
                # Or just hcat. `hcat` centers by default.
                # Projections + Cameras is usually tall? 
                # Comparison is V * H tall. 
                # Projections is 1 image? Cameras is 1 image?
                # render_projections returns list of images. Code did `hcat(*render_projections(...)[0])`.
                # So projections is horizontal strip?
                # cameras is hcat(*render_cameras). Horizontal strip.
                # right_panel is vcat(Proj, Cam). Vertical stack of strips.
                
                combined_image = hcat(comparison_panel, right_panel, align="start")
            else:
                combined_image = comparison_panel

            self.logger.log_image(
                "combined_visualization",
                [prep_image(add_border(combined_image))],
                step=self.global_step,
                caption=batch["scene"],
            )

            # Keep legacy logging if needed, or replace?
            # User asked: "Can we make 1 single image from all 4?"
            # "So the left part should be the comparison. The right part should be the projection and cameras"
            # Attempting to REPLACE the individual logs might be cleaner, but keeping them doesn't hurt.
            # I will keep the existing logs logic but wrapped/modified?
            # I replaced the block that was logging comparison, projection, cameras.
            # So I am replacing the individual logs with the combined one. 
            # WAIT. I should check if I should effectively remove the old logs.
            # The tool `multi replace` replaces the chunk.
            # My chunk covers lines 718 to 750+.
            # My replacement implements the combined logic and logs it.
            # It essentially REPLACES the separate log calls for "comparison", "projection", "cameras".
            # This seems to match the user request "make 1 single image from all 4".
