...
class GroundTruthDetectionService(DetectionService):
    ...
    def get_camera_extrinsics(self, camera_name: str) -> Optional[List[List[float]]]:
        """
        Get extrinsics for a specific camera (camera-to-world).

        Args:
            camera_name: Name of the camera

        Returns:
            4x4 matrix as list of lists, or None
        """
        for cam in self.camera_info:
            if cam.get("name") == camera_name:
                # Try common keys
                if "extrinsics" in cam:
                    ext = cam["extrinsics"]
                    if isinstance(ext, dict) and "camera_to_world_matrix" in ext:
                        return ext["camera_to_world_matrix"]
                    return ext
                if "camera_to_world" in cam:
                    return cam["camera_to_world"]
                if "pose" in cam:
                    return cam["pose"]
        return None
    ...
    def get_image_path(self, camera_name: str, frame_id: int) -> Optional[str]:
        """
        Get the image file path for a specific camera and frame.
        ...
        """
        # Handle looping
        if self.loop and self._num_frames > 0:
            frame_id = frame_id % self._num_frames

        # Construct the expected path pattern
        # Isaac Sim typically saves as: render_dir/camera_name/frame_NNNN.png
        render_dir = self.json_path.parent
        
        # Try common naming patterns
        patterns = [
            # With rgb subdirectory (Isaac Sim standard)
            render_dir / camera_name / "rgb" / f"rgb_{frame_id:04d}.png",
            render_dir / camera_name / "rgb" / f"rgb_{frame_id:05d}.png",
            render_dir / camera_name / "rgb" / f"frame_{frame_id:04d}.png",
            render_dir / camera_name / "rgb" / f"frame_{frame_id:05d}.png",
            # Without rgb subdirectory
            render_dir / camera_name / f"rgb_{frame_id:04d}.png",
            render_dir / camera_name / f"rgb_{frame_id:05d}.png",
            render_dir / camera_name / f"frame_{frame_id:04d}.png",
            render_dir / camera_name / f"frame_{frame_id:05d}.png",
            render_dir / camera_name / f"{frame_id:04d}.png",
            render_dir / camera_name / f"{frame_id:05d}.png",
        ]
