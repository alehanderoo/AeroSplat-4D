...
def render_projections(
    ...
    """
    Render orthographic projections of Gaussians from 3 axes.

    Args:
        gaussians: Gaussian primitives to render
        resolution: Output image resolution
        margin: Margin around the bounding box (fraction)
        draw_label: Whether to add axis labels
        extra_label: Additional label text
        percentile_crop: If > 0, use percentile-based bounds to exclude outliers.
                        E.g., 0.02 uses 2nd-98th percentile for tighter bounds.
    """
    ...
    if percentile_crop > 0:
        # Use percentile-based bounds to exclude outlier Gaussians (floaters)
        lower_pct = percentile_crop
        upper_pct = 1.0 - percentile_crop
        means = gaussians.means  # [B, N, 3]
        minima = torch.stack([
            torch.quantile(means[i], lower_pct, dim=0) for i in range(b)
        ])
        maxima = torch.stack([
            torch.quantile(means[i], upper_pct, dim=0) for i in range(b)
        ])
    else:
...
def render_cameras(
    batch: dict, resolution: int, frustum_scale: float = 0.05
