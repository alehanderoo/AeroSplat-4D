...
def load_pretrained_weights(model, pretrained_state_dict, strict=True):
    """Load pretrained weights, optionally filtering out mismatched shapes."""
    if strict:
        ...
        for k, v in pretrained_state_dict.items():
            if k in model_state:
                ...
                skipped_keys.append(k)
        if skipped_keys:
            print(cyan(f"Skipped {len(skipped_keys)} mismatched/missing weights"))
...
def train(cfg_dict: DictConfig):
    if cfg_dict["mode"] == "train" and cfg_dict["train"]["eval_model_every_n_val"] > 0:
        ...
        elif "objaverse" in dataset_dir:
            eval_path = "assets/evaluation_index_objaverse.json"
        else:
    ...
    if cfg.mode == "train":
        ...
        if cfg.checkpointing.pretrained_model is not None:
            ...
            load_pretrained_weights(model_wrapper, pretrained_model, strict=strict_load)
            print(
                cyan(
