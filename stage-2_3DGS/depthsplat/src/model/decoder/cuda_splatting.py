...
def render_alpha_cuda(
    ...
    """
    Render alpha/opacity channel by splatting white Gaussians against black background.
    The resulting intensity is the accumulated opacity (alpha).
    """
    b, g = gaussian_opacities.shape

    # Use constant white color (1,1,1) for all Gaussians
    fake_color = torch.ones((b, g), dtype=gaussian_opacities.dtype, device=gaussian_opacities.device)

    # Render with black background - accumulated value = alpha
    result = render_cuda(
        extrinsics,
        intrinsics,
        near,
    ...
    # Average across color channels (should all be the same)
