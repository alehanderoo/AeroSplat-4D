    ...
    def validation_step(self, batch, batch_idx):
        ...
        if not self.train_cfg.forward_depth_only:
            ...
            if "mask" in batch["context"]:
                ...
                # Build visualization: 3 rows x N columns
                context_images = batch["context"]["image"][0]  # [V, 3, H, W]
            ...
            if not self.train_cfg.no_log_projections:
                ...
                # Use percentile_crop=0.02 to exclude outlier Gaussians for tighter framing
                projections = hcat(
                    *render_projections(
                        gaussians_softmax,
                        256,
                        extra_label="(Prediction)",
                        percentile_crop=0.02,
                    )[0]
                )
                self.logger.log_image(
                    "projection",
                ...
                # Draw cameras.
                cameras = hcat(*render_cameras(batch, 256, frustum_scale=0.01))
                self.logger.log_image(
