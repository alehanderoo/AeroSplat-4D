...
def handle_found_object(
    ...
    Args:
        ...
        render_timeout (int): Number of seconds to wait for the rendering job to
            complete.
        render_normals (bool): Whether to render surface normals.
        render_depth (bool): Whether to render depth maps.
        render_mask (bool): Whether to render binary foreground masks.
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        depth_files = glob.glob(os.path.join(target_directory, "*_depth.npy"))
        normal_files = glob.glob(os.path.join(target_directory, "*_normal.npy"))
        mask_files = glob.glob(os.path.join(target_directory, "*_mask.npy"))

        missing_pngs = len(png_files) != num_renders
...
def get_example_objects() -> pd.DataFrame:
    """Returns the objects specified in the high-quality UID list."""

    if not os.path.isfile(HIGH_QUALITY_UIDS_PATH):
        raise FileNotFoundError(
            f"Missing UID list at {HIGH_QUALITY_UIDS_PATH}. "
            "Provide a valid file containing one UID per line."
        )

    with open(HIGH_QUALITY_UIDS_PATH, "r", encoding="utf-8") as f:
        requested_uids = {line.strip() for line in f if line.strip()}

    if not requested_uids:
        raise ValueError(
            f"No UIDs found in {HIGH_QUALITY_UIDS_PATH}. "
            "Ensure the file contains at least one entry."
        )

    logger.info(f"Loaded {len(requested_uids)} requested object UIDs.")
    annotations = oxl.get_annotations().copy()
    annotations["saveUid"] = annotations["fileIdentifier"].apply(get_uid_from_str)
    filtered = annotations[annotations["saveUid"].isin(requested_uids)].copy()

    missing_uids = requested_uids - set(filtered["saveUid"])
    if missing_uids:
        logger.warning(
            f"{len(missing_uids)} requested UID(s) not present in annotations."
        )

    if filtered.empty:
        raise ValueError(
            "None of the requested UIDs were found in the Objaverse-XL annotations."
        )

    logger.info(f"Found {len(filtered)} matching objects to render.")
    filtered = filtered.drop(columns=["saveUid"])
...
def render_objects(
    ...
    Args:
        ...
        render_depth (bool, optional): Render per-view depth maps. Defaults to False.
        render_normals (bool, optional): Render per-view surface normals. Defaults to
            False.
        render_mask (bool, optional): Render per-view binary masks. Defaults to False.
